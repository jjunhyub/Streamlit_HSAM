...
def get_visible_parent_for_tab(record: Dict[str, Any], node_id: str) -> Optional[str]:
    parent_id = get_parent_node_id(record, node_id)
    return parent_id


def build_node_visual_items(record: Dict[str, Any], node_id: str) -> List[Tuple[str, Image.Image, str]]:
    ...
    if assets["root_original"]:
        ...
        if img is not None:
            items.append((f"{node_id}::root_original", img, "원본 이미지"))

    overlay_img = build_overlay_on_full_image(record, node_id)
    if overlay_img is not None:
        items.append((f"{node_id}::overlay", overlay_img, f"{leaf} 오버레이"))

    if assets["mask_original"]:
        img = build_mask_original_display(record, node_id, assets["mask_original"])
        if img is not None:
            items.append((f"{node_id}::mask_original", img, f"{leaf} 원본"))

    if assets["mask"]:
        img = build_direct_display(assets["mask"])
        if img is not None:
            items.append((f"{node_id}::mask", img, f"{leaf} 마스크"))

    return items

def build_parent_visual_items(record: Dict[str, Any], node_id: str) -> List[Tuple[str, Image.Image, str]]:
    parent_id = get_visible_parent_for_tab(record, node_id)
    if not parent_id:
        assets = node_assets(record, node_id)
        if assets["root_original"]:
            img = build_direct_display(assets["root_original"])
            if img is not None:
                return [(f"{node_id}::parent_root_original", img, "원본 이미지")]
        return []

    parent_leaf = human_label(parent_id).lower()
    if parent_leaf in {"root", "others"}:
        assets = node_assets(record, node_id)
        if assets["root_original"]:
            img = build_direct_display(assets["root_original"])
            if img is not None:
    ...
    return build_node_visual_items(record, parent_id)

def build_instance_visual_items(record: Dict[str, Any], node_id: str) -> List[Tuple[str, Image.Image, str]]:
    assets = node_assets(record, node_id)
    leaf = human_label(node_id)
    items: List[Tuple[str, Image.Image, str]] = []
    for i, p in enumerate(assets["instances"], start=1):
        ...
        if img is not None:
            items.append((f"{node_id}::inst::{i}", img, f"{leaf} #{i}"))
    return items
...
def maybe_autosave_to_supabase(force: bool = False) -> None:
    if not st.session_state.get("reviewer_authenticated", False):
        return

    now_ts = time.time()
    last_ts = float(st.session_state.get("last_db_save_ts", 0.0))

    if (not force) and (now_ts - last_ts < 15.0):
        return

    reviewer = st.session_state.reviewer_id.strip() or "anonymous"

    try:
        save_annotations_to_supabase(reviewer, st.session_state.annotations)
        st.session_state["last_db_save_ts"] = now_ts
        st.session_state["last_db_save_ok"] = True
        st.session_state["last_db_error"] = ""
        st.session_state["last_db_saved_at"] = datetime.now(UTC).strftime("%H:%M:%S UTC")
...
def filter_records_by_reviewer(records: Dict[str, Any], reviewer_id: str) -> Dict[str, Any]:
    ...
    if reviewer_id not in REVIEW_RANGES:
        return {}

    sorted_ids = sorted(records.keys())
    selected_ids: List[str] = []
    for start, end in REVIEW_RANGES[reviewer_id]:
...
def ensure_annotation_bucket(image_id: str) -> Dict[str, Any]:
    ...
    if image_id not in annotations:
        annotations[image_id] = {
            "nodes": {},
            "tree_summary": {"answers": {}, "confirmed": False, "updated_at": None},
            "image_review": {"confirmed": False, "updated_at": None},
        }
...
#     path_str = str(current_annotation_path())
#     if st.session_state.get("_loaded_annotation_path") != path_str:
#         st.session_state.annotations = load_json(Path(path_str))
#         st.session_state._loaded_annotation_path = path_str
...
def missing_report(image_id: str, record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for node_id in record["actual_nodes"]:
        ...
        confirmed = node_confirmed(image_id, node_id)
        if q_missing or not confirmed:
            missing_nodes.append({"node_id": node_id, "missing_question_ids": q_missing, "confirmed": confirmed})

    tree_missing = required_missing_questions(image_id, "tree", tree_questions_for(image_id))
    return {
        "missing_nodes": missing_nodes,
        "tree_missing": tree_missing,
        "tree_confirmed": tree_summary_confirmed(image_id),
    }
...
def inject_hierarchy_global_css() -> None:
    st.markdown(
        """
        <style>
        ...
        .hier-row [data-testid="stHorizontalBlock"] {
            gap: 0 !important;
        ...
        .hier-row [data-testid="stColumn"] {
            margin: 0 !important;
            padding: 0 !important;
        }

        .hier-tree-btn button {
        ...
        .hier-tree-btn.is-idle button {
            background: rgba(255,255,255,0.03) !important;
            border: 1px solid rgba(148,163,184,0.35) !important;
            color: inherit !important;
        }

        .hier-tree-btn.is-selected button {
            background: rgba(59,130,246,0.16) !important;
            border: 1px solid #2563eb !important;
            color: #dbeafe !important;
        }

        .hier-tree-btn.is-done button {
...
def sync_questions_to_bucket(image_id: str, mode: str, questions: List[Dict[str, Any]], node_id: Optional[str] = None) -> None:
    ...
    for q in questions:
        ...
        if widget_key in st.session_state:
            answers[q["id"]] = pull_widget_value(widget_key, q)
...
def render_inspector_summary(record: Dict[str, Any], node_id: str) -> None:
    ...
    display_image_id = str(int(image_id)) if str(image_id).isdigit() else str(image_id)
    parent_label = human_label(parent_id) if parent_id else "-"
...
<div style="display:flex;flex-direction:column;justify-content:center;height:100%;min-height:30px;">
<div style="display:flex;align-items:center;flex-wrap:wrap;gap:0.35rem;">
<span class='status-pill'>N: <b>{human_label(node_id)}</b></span>
<span class='status-pill'>P: <b>{parent_label}</b></span>
<span class='status-pill'>C: <b>{children_text}</b></span>
<span class='status-pill'>D: <b>{depth}</b></span>
<span class='status-pill'><b>{pretty_path}</b></span>
</div>
...
def collect_tree_rows(record: Dict[str, Any], root_id: str) -> List[List[str]]:
    rows: List[List[str]] = []
    current = [root_id]
    while current:
        rows.append(current)
        ...
        for node_id in current:
            nxt.extend(tree_display_children(record, node_id))
        current = nxt
    return rows

def subtree_display_span(record: Dict[str, Any], node_id: str) -> int:
    children = tree_display_children(record, node_id)
    if not children:
        return 1
    return sum(subtree_display_span(record, child_id) for child_id in children)
        ...
        with st.container(key=panel_key):
            ...
            for row_idx, row in enumerate(rows):
                ...
                with st.container(key=row_key):
                    ...
                    # cols = st.columns(spec, gap="small")
                    ...
                    for part_idx, (col, (kind, _width_px, node_id)) in enumerate(zip(cols, parts)):
                        with col:
                            ...
                            selected = (
                                ...
                                node_confirmed(record["image_id"], node_id)
                                if record["nodes"][node_id]["actual"]
                                else False
                            )
...
def render_question_block(
    ...
    def render_single_question(q: Dict[str, Any]) -> None:
        ...
        if q["type"] == "single_choice":
            ...
            try:
                current_index = q["options"].index(current_value) if current_value in q["options"] else None
            except Exception:
                current_index = None
    ...
    # with col_prev:
    #     if st.button("← 이전", disabled=current_page == 0, key=f"prev_{page_state_key}"):
    #         st.session_state[page_state_key] = max(0, current_page - 1)
    #         st.rerun()

    # with col_next:
    #     if st.button("다음 →", disabled=current_page >= total_pages - 1, key=f"next_{page_state_key}"):
    #         st.session_state[page_state_key] = min(total_pages - 1, current_page + 1)
    #         st.rerun()

    # missing_now = required_missing_questions(image_id, mode, questions, node_id=node_id)
    # status_text = "완료됨" if not missing_now else f"미답변 {len(missing_now)}개"
    # st.caption(f"현재 상태: {status_text}")
...
def render_node_detail(record: Optional[Dict[str, Any]]) -> None:
    ...
    # render_inspector_summary(record, node_id)
...
def render_finalize_box(record: Dict[str, Any]) -> None:
    image_id = record["image_id"]
    st.markdown("---")
    st.markdown("#### 이미지 완료 점검")

    if st.button("이 이미지 완료 여부 확인", key=f"finalize_check_{image_id}"):
        report = missing_report(image_id, record)
        if not report["missing_nodes"] and not report["tree_missing"] and report["tree_confirmed"]:
            ensure_annotation_bucket(image_id)["image_review"] = {"confirmed": True, "updated_at": now_iso()}
            persist_annotations()
            maybe_autosave_to_supabase(force=True)
            st.success("이 image는 완료되었습니다. 왼쪽 리스트가 파란색으로 표시됩니다.")
        else:
            ensure_annotation_bucket(image_id)["image_review"] = {"confirmed": False, "updated_at": now_iso()}
            persist_annotations()
            maybe_autosave_to_supabase(force=True)
            st.error("아직 완료되지 않았습니다. 아래 누락 항목을 확인하세요.")
            if report["missing_nodes"]:
                st.markdown("**미완료 노드**")
                for item in report["missing_nodes"]:
                    if item["missing_question_ids"]:
                        labels = [q["label"] for q in node_questions_for(item["node_id"]) if q["id"] in item["missing_question_ids"]]
                        st.write(f"- {item['node_id']}: {', '.join(labels)}")
                    else:
                        st.write(f"- {item['node_id']}: 질문은 채웠지만 완료 확인 버튼을 누르지 않음")
            if report["tree_missing"] or not report["tree_confirmed"]:
                st.markdown("**전체 트리 질문**")
                if report["tree_missing"]:
                    labels = [q["label"] for q in tree_questions_for(image_id) if q["id"] in report["tree_missing"]]
                    st.write(f"- 미답변 질문: {', '.join(labels)}")
                if not report["tree_confirmed"]:
                    st.write("- 전체 트리 질문의 완료 확인 버튼을 눌러야 합니다.")
...
def render_reviewer_gate() -> None:
    st.markdown(
        ...
        ">
            ...
            <div style="
                ...
                margin-bottom: 1.2rem;
            ">
                Reviewer ID를 입력하면 할당된 이미지만 표시됩니다.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with center:
        ...
        with btn_col:
            auth_clicked = st.button("인증", type="primary", use_container_width=True)
        ...
        if auth_clicked:
            ...
            elif reviewer_input not in REVIEW_RANGES:
                st.session_state.reviewer_authenticated = False
...
def render_sidebar(records: Dict[str, Any]) -> None:
    with st.sidebar:
        ...
        if st.button("💾 지금 DB에 저장", use_container_width=True):
            persist_annotations()
            maybe_autosave_to_supabase(force=True)

        if st.session_state.get("last_db_save_ok") is True:
            saved_at = st.session_state.get("last_db_saved_at", "")
            st.caption(f"마지막 DB 저장: {saved_at}")
        elif st.session_state.get("last_db_save_ok") is False:
            st.error(f"DB 저장 실패: {st.session_state.get('last_db_error', '')}")


        # st.markdown("---")
        # st.write(f"할당된 image 개수: **{len(records)}**")
        # st.write(f"dataset root: `{DATASET_ROOT}`")
        # st.write(f"annotation 파일: `{current_annotation_path()}`")
...
def main() -> None:
    ...
    if st.session_state.selected_image_id not in records:
        ...
        if st.session_state.selected_image_id:
            first_nodes = records[st.session_state.selected_image_id]["actual_nodes"]
            st.session_state.selected_node_id = first_nodes[0] if first_nodes else None
    ...
    if selected_record and st.session_state.selected_mode == "node":
        if st.session_state.selected_node_id not in selected_record["nodes"]:
            ...
            st.session_state.selected_node_id = actual_nodes[0] if actual_nodes else None
